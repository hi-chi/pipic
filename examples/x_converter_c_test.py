...
@cfunc(add_particles)
def density_profile1(r, data_double, data_int):  # callback function
    return density * (abs(r[0]) < 32 * debye_length) * (r[0] < 0)
...
sim.add_particles(
    ...
    mass=electron_mass,
    temperature=temperature,
    density=density_profile1.address,
)
...
def plot_xpx():
    ...
    xpx_dist.fill(0)
    sim.particle_loop(
        name="electron1", handler=xpx.address, data_double=addressof(xpx_dist)
    )

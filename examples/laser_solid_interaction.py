...
amplitude_a0 = 100
omega = 2 * np.pi * light_velocity / wavelength
...
supRate = 1 - np.exp(np.log(0.01) / (wavelength / (light_velocity * time_step)))
...
def field_callback(ind, r, E, B, data_double, data_int):
    if data_int[0] == 0 or ymax - r[1] < boundarySize:
        ...
        lCoord = np.sin(incidenceAngle) * r[0] - np.cos(incidenceAngle) * r[1] + \
                 + (arrivalDelay - data_int[0]*time_step) * light_velocity
...
#-----------------------------seting plasma-------------------------------------
N_cr = electron_mass * omega ** 2 / (4 * np.pi * electron_charge ** 2)
density = 100*N_cr

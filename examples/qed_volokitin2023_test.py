...
gamma = 2e+5
E_cr = consts.electron_mass**2*consts.light_velocity**3/(abs(consts.electron_charge)*consts.hbar)
B_y = 0.2*E_cr

t_rad = 3.85*gamma**(1/3.0)*(E_cr/B_y)**(2/3)*consts.hbar**2/(consts.electron_mass*consts.light_velocity*consts.electron_charge**2)
t_sim = 8*t_rad
N_steps = 32
...
def N_e_ep_cb(r, p, w, id, data_double, data_int):
    if math.sqrt(p[0]**2 + p[1]**2 + p[2]**2) > (1e-3)*gamma*consts.electron_mass*consts.light_velocity:
        data_double[0] += w[0]
    else:
        w[0] = 0
def get_N_e_ep():
    N_e_ep[0] = 0
    sim.particle_loop(name='electron', handler=N_e_ep_cb.address, data_double=pipic.addressof(N_e_ep))
    sim.particle_loop(name='positron', handler=N_e_ep_cb.address, data_double=pipic.addressof(N_e_ep))
    return N_e_ep[0]
...
for i in range(Nsteps):
    ...
    remove_low_en_ph()
    if i%(N_steps/32) == 0:
        print(i/(N_steps/32), '/', Nsteps, ': N_{ee+} =', get_N_e_ep()/N0, ', N_{macro-particles} =', sim.get_number_of_particles())

...
- `init(solver, nx, xmin, xmax, ny=1, ymin=-0.5, ymax=0.5, nz=1, zmin=-0.5, zmax=0.5)` allocates data for the field grid and creates a container for particles
    ...
    - `solver` provides a way to select one of the following solvers to be used for the simulation:
        ...
        - `emc2` is an electromagnetic energy-conserving solver
        - `electrostatic_1d` is a 1D electrostatic solver
        - `electrostatic_1d_implicit` is a 1D implicit electrostatic solver

Interfaces and functions of containers
--

- `add_particles(name, number, charge, mass, temperature, density, data_double = 0, data_int = 0)` adds particles of new or existing type (in case of an exact match)
    ...
    - `density` is the address of a function that gives density as a function of coordinates: `density(r, data_double, data_int)` (decorator `@cfunc(types.add_particles)` must be placed before function definition)
...
- `particle_loop(name, handler, data_double = 0, data_int = 0, use_omp = False)` makes a loop over particles of type `name` and applies action `handler`
    - `name` defines the type of particles to be processed
    - `handler` is the address of a function that defines actions to be performed with each particle: `handler(r, p, w, id, data_double, data_int)` (decorator `@cfunc(types.particle_loop)` must be placed before function definition)
    ...
    - `use_omp` defines whether to make a parallel loop (via OpenMP); if `use_omp = True` the `handler` must be thread-safe

- `field_loop(handler, data_double = 0, data_int = 0, use_omp = False)` makes a loop over grid nodes
    - `handler` is the address of a function that defines the action on field components: `handler(ind, r, E, B, data_double, data_int)` (decorator `@cfunc(types.field_loop)` must be placed before function definition)
...
- `custom_field_loop(handler, number_of_iterations = 0, it2r = 0, field = "", data_double = 0, data_int = 0)` makes a loop over a subset of locations, at which the selected field is interpolated and sent to `handler`. The implementation of this method may vary between solvers. Check the documentation of the specific solver for details. 
    - `handler` is the address of a function that defines how field_data is saved using `data_double` and `data_int`: `handler(it, r, field, data_double, data_int)` (decorator `@cfunc(types.custom_field_loop)` must be placed before function definition)
    ...
    - `it2r` is the address of a function that defines the locations, at which the field interpolated is needed: `handler(it, r, data_double, data_int)` (decorator `@cfunc(types.it2r)` must be placed before function definition)
        - `it[0]` is the index of the subset element (read only)
...
- `add_handler(name, subject = "", handler = 0, field_handler = 0, data_double = 0, data_int = 0)` incorporates an extension, which can add and remove particles or modify fields during state advancement (see [Making extensions](../EXTENSIONS.md))
    - `name` is the name of the extension
    - `subject` is a string list of particle types to be affected, `all_types` can be used to affect all particle types, and `cells` indicates that the handler is to be called for all cells (even if empty)
    - `handler` is the address of the function to act on particles/cells (see [Making extensions](../EXTENSIONS.md))
    - `field_handler` is the address of the function to act on fields
    - `data_double` and `data_int` are addresses for exchanging data between `handler` function and the remaining Python script

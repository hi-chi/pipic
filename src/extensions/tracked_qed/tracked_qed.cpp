/*-------------------------------------------------------------------------------------------------------
This file is an extesion of qed_gonoskov2015 that that is made to track the value of chi at the instance 
of producing a new electron, positron or photon. The value of chi is placed in particle::a.
tracked_qed, Copyright 2024 Arkady Gonoskov
---------------------------------------------------------------------------------------------------------
tracked_qed is free software: you can redistribute it and/or modify it under the terms of the GNU 
General Public License as published by the Free Software Foundation, either version 3 of the License, or 
(at your option) any later version.

tracked_qed is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without 
even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General 
Public License for more details.

You should have received a copy of the GNU General Public License along with tracked_qed. If not, se
...
const string name = "tracked_qed";
static int attibuteIndex; // the index of attribute, where the chi value at generation is placed
    ...
    void handleCascade(double E_E_cr, double timeStep, int seed_ip, cellInterface &CI){ // E_E_cr = H_eff/schwingerField
        ...
        for(int is = 0; is < numberOfSubSteps; is++){
            ...
            for(int ip = 0; ip < nParticles; ip++){
                double chi = E_E_cr*cParticle[ip].gamma;
...
int64_t handler(int electronType_, int positronType_, int photonType_, 
                double probabilityThreshold_, double probabilitySubcycle_, int attibuteIndex_){
    attibuteIndex = attibuteIndex_;
    if((attibuteIndex + 1)*8 > int(sizeof(particle)) - 64) {
        cout << "pipic: tracked_qed: Error: cannot place chi at attribute_index = " << attibuteIndex << ", use or complie pipic version with extended memory of struct particle." << endl;
        exit(0);
    }
...
PYBIND11_MODULE(_tracked_qed, object) {
    object.attr("name") = name;
    object.def("handler", &handler, py::arg("electron_type"), py::arg("positron_type"), py::arg("photon_type"),
                py::arg("probability_threshold") = 1e-3, py::arg("probability_subcycle") = 0.1, py::arg("attibute_index") = 0);

...
if __name__ == '__main__':
    ...
    nx, xmin, xmax = 2**5, -6*pulseWidth_x, 6*pulseWidth_x
    ny, ymin, ymax = 2**5, -6*spotsize, 6*spotsize
    nz, zmin, zmax = 2**7, -15*spotsize, 5*spotsize
    ...
    def initiate_density(r, data_double, data_int):# Just to add electrons to particle list
        ...
        r_min = r_rel 
        r_max = r_rel + dz
        if (r[0] > r_min and r[0] < r_max) or (r[0] > zmax - (zmin - r_min)) or (r[0] < zmin + (r_max - zmax)):
            return n0*1e-10
        else:
            return 0 

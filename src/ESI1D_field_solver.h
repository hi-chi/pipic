...
struct ESI1DFieldSolver: public field_solver // 1D electrostatic solver (Ex only)
{
    string type;
    vector<double> Ex; // electric field at the nodes
    vector<double> Jx; // current density at the nodes
    ESI1DFieldSolver(simulationBox box): field_solver(box){
        type = "ES1D";
    ...
    // getEB is a non-mandatory method for field solver interface, but is needed for the particle pusher in the electrostatic model to retrieve electric field at the particle position.
